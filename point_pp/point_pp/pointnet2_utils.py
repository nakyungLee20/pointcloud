...
class PointNetSetAbstractionMsg1(nn.Module):
    def __init__(self, npoint, radius_list, nsample_list, in_channel, mlp_list):
        ...
        for i in range(len(mlp_list)):
            ...
            for j, out_channel in enumerate(mlp_list[i]):
                if j ==0:
                    convs.append(nn.Conv2d(last_channel-3, out_channel, 1))
                else:
    ...
    def forward(self, xyz, points):
        ...
        xyz = xyz.permute(0, 2, 1)
        if points is not None:
            points = points.permute(0, 2, 1) # 12, 2806 (# data), 3
        ...
        for i, radius in enumerate(self.radius_list):
            ...
            if points is not None:
                grouped_points = index_points(points, group_idx) # [12, 512, 32, 3]
...
class PointNetFeaturePropagation1(nn.Module):
    def __init__(self, in_channel, mlp):
        ...
        for i, out_channel in enumerate(mlp):
            if i == 0:
                self.mlp_convs.append(nn.Conv1d(last_channel-2, out_channel, 1))
            else:
                self.mlp_convs.append(nn.Conv1d(last_channel, out_channel, 1))
            self.mlp_bns.append(nn.BatchNorm1d(out_channel))
            last_channel = out_channel
